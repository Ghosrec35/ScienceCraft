...
public class BlockCatalyzer extends BlockMachine
{
	private Random random = new Random();
	
	private final boolean isActive;
	
	public static boolean keepCatalyzerInventory = false;
	
	public BlockCatalyzer(int id, boolean bool) 
	{
		super("Catalyzer", id, Material.rock);
		this.isActive = bool;
	...
	{
		return ScienceCraft.registry.catalyzerInactive.blockID;
	}
	...
	public boolean onMachineActivated(World world, int x, int y, int z, EntityPlayer player)
	{
		...
		if(!world.isRemote)
		{
			player.openGui(ScienceCraft.instance(), 1, world, x, y, z);
			return true;
		}
		return true;
	}

	public static void updateRefinerBlockState(boolean b, World world, int xCoord, int yCoord, int zCoord) 
	{
		int block = world.getBlockMetadata(xCoord, yCoord, zCoord);
		TileEntity te = world.getBlockTileEntity(xCoord, yCoord, zCoord);
		keepCatalyzerInventory = true;
		
		if(b)
		{
			world.setBlockWithNotify(xCoord, yCoord, zCoord, ScienceCraft.registry.catalyzerActive.blockID);
		}
		else
		{
			world.setBlockWithNotify(xCoord, yCoord, zCoord, ScienceCraft.registry.catalyzerInactive.blockID);
		}
		
		keepCatalyzerInventory = false;
	...
	{
		if(!keepCatalyzerInventory)
		{
			TileEntityRefiner ter = (TileEntityRefiner)world.getBlockTileEntity(x, y, z);
